...
let circles = [];
const maxRadius = 50; // maximum radius for smaller circles
...
function makeCircles() {
  ...
  let cols = floor(width / (bigCircleRadius * 2 + 25));
  let rows = floor(height / (bigCircleRadius * 2 + 25));

  // nested loops to create circle objects for each column and row
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      // calculate position for the big circle
      let x = bigCircleRadius + 45 + col * (bigCircleRadius * 2 + 45);
      let y = bigCircleRadius + 45 + row * (bigCircleRadius * 2 + 45);
      ...
      circles.push(
        new CircleCenter(x, y, maxRadius * 0.2, color(hueCenter, 100, 50, 0.7))
      ); // create circle center object and add to array
    }
  }
}
  ...
  show() {
    ...
    for (let k = 0; k < splitCircle; k++) {
      ...
      for (let j = 0; j < spectrum.length; j+=4) {
        ...
        vertex(
          (y + y_r + r) * cos(x + curveBase * k) + this.pos.x,
          (y + y_r + r) * sin(x + curveBase * k) + this.pos.y
        );
      }
      endShape();
    }
